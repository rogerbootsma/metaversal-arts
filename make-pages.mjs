...
import {projectHubs} from './dist/project-links.js';
...
const pages={
 ...
 <h2>Worlds in orbit</h2><p>An original procedural planet with flowing plum and copper bands, slow white volumetric cloud ribbons, a soft blue atmosphere and twelve textured moons. Inclined, shallow volumetric rings carry rotating dust, small rock debris and occasional comet trails.</p>
...
const projectHead=home.slice(home.indexOf('<head>'),home.indexOf('</head>')+7)
 ...
 .replace('</head>','<link rel="stylesheet" href="./projects.css?v=atmosphere-20260917-1">\n<script type="module" src="./projects-scene.js?v=atmosphere-20260917-1"></script>\n</head>');
const projectHeader=header.replace('href="./projects.html"','href="./projects.html" aria-current="page"');
const items=projects.map(({name,type},i)=>`<li><span class="project-number">${String(i+1).padStart(2,'0')}</span><span class="project-entry"><span data-project-name data-project-type="${type}">${name}</span><small>${projectTypes[type]}</small>${projectHubs[name]?`<span class="directory-destinations">${projectHubs[name].links.map(({label,url})=>`<a href="${url}" target="_blank" rel="noopener noreferrer" aria-label="${projectHubs[name].name} ${label} (opens in a new tab)">${label}</a>`).join('')}</span>`:''}</span><img src="./star-nine.svg" width="18" height="18" alt=""></li>`).join('\n');
...
<main id="main">
 <section class="constellation" aria-labelledby="projects-title">
  ...
  <div class="orbit-categories" role="group" aria-label="Highlight a project group" hidden>${projectTypes.map((name,i)=>`<button type="button" data-orbit="${i}" aria-pressed="false"><span aria-hidden="true">0${i+1}</span>${name}</button>`).join('')}</div>
  <div class="cloud-stage" id="cloud-stage" role="group" aria-label="Project constellation. ASimulation and BlenderMonk have destination menus."><canvas id="project-cloud" aria-hidden="true"></canvas><svg id="orbit-connectors" aria-hidden="true"></svg><div id="orbit-labels"></div></div>
